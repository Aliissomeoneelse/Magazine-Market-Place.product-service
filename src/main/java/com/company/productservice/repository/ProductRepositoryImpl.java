...
public class ProductRepositoryImpl {
    ...
    public Page<Product> getProduct(Map<String, String> params) {
        int size = 10, page = 0;
        ...
        String countQuery = "select count(p.id) from Product p where 1=1 ";
    ...
    private StringBuilder builderParams(Map<String, String> params) {
        ...
        if (params.containsKey("image")) {
            stringBuilder.append(" And p.image = :image");
        }
        if (params.containsKey("imageId")) {
            stringBuilder.append(" And p.imageId = :imageId");
        }
        if (params.containsKey("expiredAt")) {
            stringBuilder.append(" And p.expiredAt = :expiredAt");
        }
    ...
    private void setParams(Query query, Map<String, String> params) {
        ...
        if (params.containsKey("image")) {
            query.setParameter("image", params.get("image"));
        }
        if (params.containsKey("imageId")) {
            query.setParameter("imageId", params.get("imageId"));
        }
        if (params.containsKey("expiredAt")) {
            query.setParameter("expiredAt", params.get("expiredAt"));
        }
    }
